...
class ucb_algo:
    ...
    def __init__(self, team, Q, num_species, num_tasks, num_traits, demo, total_ite, delta):
        ...
        while ite <= self.ite_to_run and self.actual_ite < self.T-1:
            ...
            if (self.beta<0).any() or (self.beta==np.inf).any():
                ...
                for task in range(temparg.shape[0]):
                    ...
                    #arm[task] = self.feasible_trait_space[int(temparg[task])]
    ...
    def get_task_score(self, x):
        ...
        #adding noise to calculated score, sigma_noise here is for np.random.seed(42)
    ...
    def argmax_ucb(self, min_task=None):
        """
        ...
        for task in range(sigma.shape[0]):
            ...
            #t_[task,:] = sigma[task, :]*5*np.sqrt(beta)[task]
                if min_task == task:
                    t_[task,:] = sigma[task, :]*(random. randint(1, 3))*np.sqrt(beta)[task]
                #else:
                    #t_[task,:] = sigma[task, :]*(random. randint(3, 5))*np.sqrt(beta)[task]
            else:
                t_[task,:] = sigma[task, :]*(random. randint(1, 3))*np.sqrt(beta)[task]
                #t_[task,:] = sigma[task, :]*5*np.sqrt(beta)[task]
