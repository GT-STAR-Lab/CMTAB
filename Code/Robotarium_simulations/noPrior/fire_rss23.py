...
def fire_task(X, Q, visualize= False):
    ...
    if visualize:
        fire = plt.imread('fire.png')
        x_img = np.linspace(0.0, 1, fire.shape[1])
        y_img = np.linspace(0.0, 1, fire.shape[0])

        fire_handle = r.axes.imshow(fire, extent=(-1.2, -0.7, 0.5, 0.9))
        ...
        box = plt.imread('water.png')
        x_img = np.linspace(0.0, 1, box.shape[1])
        y_img = np.linspace(0.0, 1, box.shape[0])

        box_handle = r.axes.imshow(box, extent=(1.2, 1.6, 0.6, -0.2))
    ...
    fire_reached = np.ones(N)

    trait_values = []
    for i in range(4):
        for k in range(X[0][i]):
            trait_values.append(Q.T[1][i])
    ...
    fire_point_1 = np.random.uniform(low=-1.2, high=-0.7, size=(N,))
    fire_point_2 = np.random.uniform(low=0.5, high=0.9, size=(N,))
    fire_point = np.array([fire_point_1,fire_point_2])
    ...
    if visualize:    
        ...
        text_counter = r.axes.text(1.4,0.9,str(total_fire),fontsize=20, color='k',fontweight='bold',horizontalalignment='right',verticalalignment='top',zorder=10)

    # Perform the simulation unitl fire gets cleared or to the certain counter value
    x_goal = np.copy(clearing_point)
    # print("Going to goal", x_goal)
    iter_count = 0
    while(not fire_cleared):
        ...
        for i in status_changed:
            if fire_reached[i] == 1:
                total_fire -= trait_values[i]
                if total_fire < 0:
                    total_fire = 0
                if visualize:
                    text_counter.remove()
                    text_counter = r.axes.text(1.4,0.9,str(total_fire),fontsize=20, color='k',fontweight='bold',horizontalalignment='right',verticalalignment='top',zorder=10)
                #print("Fire needed to clear", str(total_fire))

            fire_reached[i] = 1-fire_reached[i] 

            if(fire_reached[i] == 0):
                x_goal[:,i] = fire_point[:,i]
            else:
                x_goal[:,i] = clearing_point[:,i]
            
            if (total_fire < 1) and (fire_reached[i] == 1):
                fire_cleared = True
    ...
    return ((reward_value-total_fire)/reward_value)
...
if __name__ ==  '__main__':
    ...
    Q= np.array([[ 0.1, 4, 1, 0.18],
        ...
        [0.1, 1, 2, 0.54],
        [0.2, 2, 2, 0.24]])
    reward = fire_task(X, Q, visualize= False)
    print(reward)
