...
class stmria:
    ...
    def zoomin_algo(self, optimal_score=None):
        ...
        self.zooming_dict = {'ball_centres': [], 'conf_radius': [], 'num_pulled':[], 'allocation':[]}
        ...
        disc_int = 0.02 #discretization interval
        ...
        self.init_zooming_dict()
        print("Done initialization of zooming dict")
        self.arms_in_conf_ball()
        print("Done config ball")
        ...
        del self.learned_prior_mesh
        return self.scores, self.highestArm, self.highest_demo, self.high_score_f_alloc, self.inc_lml
    ...
    def init_zooming_dict(self):
        '''
        Dictionary to store activated arms and associated values
        '''
            
        for i in range(self.feasible_trait_space.shape[0]):
            allocation = self.int_least_squares_total(self.feasible_trait_space[i])
            #A confidence ball always has at least one feasible allocation
            if (allocation != 0).any():
                self.zooming_dict['ball_centres'].append(self.feasible_trait_space[i])
                self.zooming_dict['conf_radius'].append(self.confidence_radius(0,0))
                self.zooming_dict['num_pulled'].append(0)
                self.zooming_dict['allocation'].append(allocation)
 

    def xgrid_in_conf_ball(self, conf_centres, conf_radius):
        '''
        For confidence centres of every task in a confidence ball,
        finds values in X_grid that are within the confidence ball
        conf_centres: num_tasks X num_traits
        conf_radius: 1 dimensional
        '''
        X_grid_traits = [[],[],[]]
        X_grid_traitsarg = [[],[],[]]
        ...
        for task in range(self.num_tasks):
            norm = [1,1,1,1,1]
            argnorm = [0,0,0,0,0]
            for j in range(self.X_grid.shape[0]):
                
                #The following two lines of code find out all X_grid values in the confidence ball
                #This requires more computational resources
                #if np.linalg.norm(X_grid[j]-conf_centres[task])<=conf_radius:
                    #X_grid_traits[task].append(X_grid[j])
                    #X_grid_traitsarg[task].append(j)
                
                if np.linalg.norm(self.X_grid[j]-conf_centres[task],2)<=norm[0]:
                    norm[0]= np.linalg.norm(self.X_grid[j]-conf_centres[task],2)
                    argnorm[0]=j
                if np.linalg.norm(self.X_grid[j]-(conf_centres[task]+conf_radius),2)<=norm[1]:
                    norm[1]= np.linalg.norm(self.X_grid[j]-(conf_centres[task]+conf_radius),2)
                    argnorm[1]=j
                if np.linalg.norm(self.X_grid[j]-(conf_centres[task]-conf_radius),2)<=norm[2]:
                    norm[2]= np.linalg.norm(self.X_grid[j]-(conf_centres[task]-conf_radius),2)
                    argnorm[2]=j
                if np.linalg.norm(self.X_grid[j]-(conf_centres[task]+(0.5*conf_radius)),2)<=norm[3]:
                    norm[3]= np.linalg.norm(self.X_grid[j]-(conf_centres[task]+(0.5*conf_radius)),2)
                    argnorm[3]=j
                if np.linalg.norm(self.X_grid[j]-(conf_centres[task]-(0.5*conf_radius)),2)<=norm[4]:
                    norm[4]= np.linalg.norm(self.X_grid[j]-(conf_centres[task]-(0.5*conf_radius)),2)
                    argnorm[4]=j
            X_grid_traits[task].append(self.X_grid[argnorm[0]])
            X_grid_traits[task].append(self.X_grid[argnorm[1]])
            X_grid_traits[task].append(self.X_grid[argnorm[2]])
            X_grid_traits[task].append(self.X_grid[argnorm[3]])
            X_grid_traits[task].append(self.X_grid[argnorm[4]])
            X_grid_traitsarg[task].append(argnorm[0])
            X_grid_traitsarg[task].append(argnorm[1])
            X_grid_traitsarg[task].append(argnorm[2])
            X_grid_traitsarg[task].append(argnorm[3])
            X_grid_traitsarg[task].append(argnorm[4])
        '''
        for task in range(self.num_tasks):
            norm = [1,1,1]
            argnorm = [0,0,0]
        ...
        input = []
        for task in range(self.num_tasks):
            input.append(np.arange(0, len(X_grid_traits[task]), 1))
        meshgrid = np.array(np.meshgrid(input[0], input[1], input[2]))
        temp_grid = meshgrid.reshape(meshgrid.shape[0], -1).T
        feasible_trait_agg = np.zeros([temp_grid.shape[0], self.num_tasks, self.num_traits])
        feasible_trait_aggarg = np.zeros([temp_grid.shape[0], self.num_tasks, 1])
        for i in range(temp_grid.shape[0]):
            for task in range(self.num_tasks):
                 feasible_trait_agg[i, task, :] = X_grid_traits[task][temp_grid[i, task]]
                 feasible_trait_aggarg[i, task, :] = X_grid_traitsarg[task][temp_grid[i, task]]
        indices = []
        for i in range(feasible_trait_agg.shape[0]):
            if (np.sum(feasible_trait_agg[i], axis=0)<=1).all() and (feasible_trait_agg[i]!=0).all():
                indices.append(i)
        feasible_trait_agg =  feasible_trait_agg[indices, :, :]
        feasible_trait_aggarg =  feasible_trait_aggarg[indices, :, :]
        return feasible_trait_agg, feasible_trait_aggarg

    def arms_in_conf_ball(self):
        self.zooming_dict['trait_aggs'] = []
        self.zooming_dict['trait_aggsarg'] = []
        for i in range(len(self.zooming_dict['ball_centres'])):
            trait_agg, trait_aggarg = self.xgrid_in_conf_ball(self.zooming_dict['ball_centres'][i],                                                                                           self.zooming_dict['conf_radius'][i])
            self.zooming_dict['trait_aggs'].append(trait_agg)
            self.zooming_dict['trait_aggsarg'].append(trait_aggarg)
        

    def calc_selection_index(self, trait_aggsarg, radius, ite, min_task=None):
        '''
        trait_aggsarg: num of possible trait aggregations X num_tasks
        '''
        ...
        for task in range(sigma.shape[0]):
            #t_[task,:] = sigma[task, :]*np.sqrt(beta)[task]
            t_[task,:] = sigma[task, :]*(beta)[task]*0.1
        temp = np.zeros ([trait_aggsarg.shape[0],1])
        for i in range(trait_aggsarg.shape[0]):
            temp[i] = 2*radius 
            for task in range(sigma.shape[0]):
                if min_task:
                    if task==min_task:
                        temp[i] += min_fac*(mu[task,int(trait_aggsarg[i, task])] + t_[task,int(trait_aggsarg[i, task])])
                    else:
                        temp[i] += mu[task,int(trait_aggsarg[i, task])] + t_[task,int(trait_aggsarg[i, task])]
                else:
                    temp[i] += mu[task,int(trait_aggsarg[i, task])] + t_[task,int(trait_aggsarg[i, task])]
        return np.max(temp), np.argmax(temp)

    def select_arm(self, ite, min_task=None):
        self.zooming_dict['selection_index'] = []
        self.zooming_dict['selection_indexarg'] = []
        self.optimal_beta_selection(ite)
        #print(beta)
        for i in range(len(self.zooming_dict['ball_centres'])):
            if self.zooming_dict['conf_radius'][i] !=0:
                trait_aggsarg = np.array(self.zooming_dict['trait_aggsarg'][i])
                selection_ind, selection_indarg = self.calc_selection_index(trait_aggsarg, self.zooming_dict['conf_radius'][i], ite, min_task)
                self.zooming_dict['selection_index'].append(selection_ind)
                self.zooming_dict['selection_indexarg'].append(selection_indarg)
            else:
                self.zooming_dict['selection_index'].append(-1)
                self.zooming_dict['selection_indexarg'].append(-1)

            selected_arm = np.argmax(np.array(self.zooming_dict['selection_index']))

        return selected_arm, self.zooming_dict['trait_aggs'][selected_arm][self.zooming_dict['selection_indexarg'][selected_arm]]

    def add_arm(self, new_centres, new_rad, allocation):
        self.zooming_dict['ball_centres'].append(new_centres)
        self.zooming_dict['conf_radius'].append(new_rad)
        self.zooming_dict['num_pulled'].append(0)
        self.zooming_dict['allocation'].append(allocation)
        index_added = len(self.zooming_dict['conf_radius'])-1
        trait_agg, trait_aggarg = self.xgrid_in_conf_ball(self.zooming_dict['ball_centres'][index_added], self.zooming_dict['conf_radius'][index_added])
        self.zooming_dict['trait_aggs'].append(trait_agg)
        self.zooming_dict['trait_aggsarg'].append(trait_aggarg)

    def activate_more_arms(self, neighbour_arm, ite):
        new_rad = self.confidence_radius(ite, self.zooming_dict['num_pulled'][neighbour_arm])
        radius_diff = self.zooming_dict['conf_radius'][neighbour_arm] - new_rad

        new_centres = self.zooming_dict['ball_centres'][neighbour_arm]+(new_rad+(radius_diff/2))
        if (new_centres>=self.low_margin).all() and (new_centres<=self.high_margin).all():
            allocation = self.int_least_squares_total(new_centres)
            #A confidence ball always has at least one feasible allocation
            if (allocation != 0).any():

                self.add_arm(new_centres, new_rad, allocation)
    ...
    def beta_nestucb(self, ite):
        ...
        for task in range(self.num_tasks):
            beta.append ((0.08 *np.log(((T-ite)**2)*2*(np.pi**2)/(3*self.delta[task]))) + (2*d*np.log(((T-ite)**2)*d*b*r*np.sqrt(np.log(4*d*a/self.delta[task])))))
        scaling = 5+((ite)*(10-5))/(T-ite)
        beta = np.array(beta)/ (2*(scaling**2))
        #print("beta: ",np.sqrt(beta))
        beta_ = np.reshape(beta, (1, beta.shape[0]))
        return beta_

    def learn_new_prior(self, new_feasible_):
        
        prior = {'gp': [], 'kernel': [], 'mu':[], 'sigma':[]} 
        for task in range(self.num_tasks):
            prior['gp'].append(GaussianProcessRegressor(kernel=self.kernel1, normalize_y=True))
            prior['gp'][task].fit(self.taskAssign[task, :, :], self.taskScore[task, :])
            prior['kernel'].append(prior['gp'][task].kernel_)
            mu_, sigma_ = prior['gp'][task].predict(new_feasible_[:,task,:], return_std=True)
            prior['mu'].append(mu_)
            prior['mu'][task]=np.reshape(np.array(prior['mu'][task]), (len(mu_),1))
            prior['sigma'].append(sigma_)
            prior['sigma'][task]=np.reshape(np.array(prior['sigma'][task]), (len(mu_),1))
    ...
    def select_arms_and_zoomin(self):
        ...
        for ite in range(self.T):
            ...
            arm, arm_agg = self.select_arm(ite)
            print(arm_agg)
            allocation = self.int_least_squares_total(arm_agg, True)
            print(allocation)
            #allocation_ = int_least_squares_total(zooming_dict['ball_centres'][arm], Q, team)
            if (allocation != 0).any() and (np.sum(allocation, axis=1)!=0).all():
                rad_ = self.zooming_dict['conf_radius'][arm]
                self.zooming_dict['allocation'][arm] = allocation
                self.zooming_dict['num_pulled'][arm] += 1
                self.activate_more_arms(arm, ite)
                self.zooming_dict['conf_radius'][arm] =  self.confidence_radius(ite, self.zooming_dict['num_pulled'][arm])
                print("radius: ",self.zooming_dict['conf_radius'][arm])
                self.zooming_dict['trait_aggs'][arm], self.zooming_dict['trait_aggsarg'][arm] = self.xgrid_in_conf_ball(self.zooming_dict['ball_centres'][arm], self.zooming_dict['conf_radius'][arm])
                ...
                self.zooming_dict['num_pulled'][arm] += 1
                self.activate_more_arms(arm, ite)                
                self.zooming_dict['conf_radius'][arm] = 0
            if any((x <= self.rad_lim and x > 0) for x in self.zooming_dict['conf_radius']) :
                ...
                break
        #using the picked centres, add more trait agg to the zoom-in dict
        self.ite = len(self.scores)
    ...
    def feasible_allocs(self):
        ...
        #avg_score = avg_score/len(feasible_allocations)
        ...
        while finding_best_alloc and tries<(self.T-1):
            tries+=1
            mu = mu/np.max(mu, axis=0)
            sigma = sigma/np.max(sigma, axis=0)
            self.optimal_beta_selection( tries)
            beta = self.beta[0][0]
            index= np.argmax(np.sum(mu, axis=1)+(beta*0.1*np.sum(sigma, axis=1)))
            alloc_ = self.feasible_allocations[index]
            score = self.get_task_score(alloc_)
            self.scores.append(score)
            trait_agg = (alloc_@self.Q)/(self.team@self.Q)
            self.taskAssign = np.insert(self.taskAssign, self.taskAssign.shape[1], trait_agg, axis= 1)
            self.taskScore = np.insert(self.taskScore, self.taskScore.shape[1], score, axis= 1)
            for task in range(self.num_tasks):
                mu[:, task], sigma[:, task] = self.learned_prior_mesh['gp'][task].predict(zoomed_traitspace[:,task,:], return_std=True)
