...
def get_team_dets(t):
    ...
    if t == 100:
        ...
        [0.2, 2, 2, 0.24]])
        team = [4, 4, 4, 3]
        optimal_alloc = np.array([[4, 0, 0, 1],
                                  [0, 4, 0, 1],
                                  [0, 0, 4, 1]])
        optimal_score = np.array([0.68, 0.53, 0.6]) 
...
def main():
    ...
    num_species = 4 
    num_tasks = 3  #move debris, search an environment, fire dousing
    num_traits = 4 #speed,water,payload,sensing radius
    og_team = [7, 3, 4, 6]
    csv_filename = "demos" + str(teamNo)+"_"+time.strftime("%m%d_%H%M%S")+ ".csv"
    num_demo = 100 #approximate number of demos that will be generated
    ...
    for rollout in range(num_rounds):
        ...
        experiment_ucb = ucb_algo(new_team, new_Q, num_species, num_tasks, num_traits, total_ite, delta)
        ucb_scores, ucb_high, lml_ucb_ = experiment_ucb.pull_maxucb()
        lml_ucb.append(np.array(lml_ucb_))
        ucb_scores = np.array(ucb_scores) 
        
        save_in_csv(ucb_scores, csv_filename[:-4]+"_nu"+str(rollout)+".csv")
        ...
        experiment = stmria_ind(new_team, new_Q, num_species, num_tasks, num_traits, total_ite, delta, rad_limit, start_time)
        ...
        save_in_csv(indzoomin_scores, csv_filename[:-4]+"_ni"+str(rollout)+".csv")
        ...
        experiment = stmria(new_team, new_Q, num_species, num_tasks, num_traits, total_ite, delta, rad_limit, start_time)
        #if rollout==0:         
        ...
        save_in_csv(zoomin_scores, csv_filename[:-4]+"_nz"+str(rollout)+".csv")
        ...
        experiment_rndm = stmria(new_team, new_Q, num_species, num_tasks, num_traits, total_ite, delta, rad_limit, start_time)
        ...
        save_in_csv(rndm_scores, csv_filename[:-4]+"_nr"+str(rollout)+".csv")
    ...
    if ucb_high_all:
        ...
        plotting_realtimescore(zoomin_scores_all, ucb_scores_all, rndm_scores_all, optimal_score, high_demo, csv_filename[:-4]+"_realtime.png")
        plotting_minscore(zoomin_min_all, ucb_min_all, rndm_min_all, csv_filename[:-4]+"_min.png")
        plotting_indscore(zoomin_ind_all, ucb_ind_all, rndm_ind_all, csv_filename[:-4], optimal_score)
    else:
        plotting_high(zoomin_high_all, None, rndm_high_all, csv_filename[:-4]+"_high.png", optimal_score, high_demo, high_f)
